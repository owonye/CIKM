...
def answer_f1(answer_a: str, answer_b: str) -> float:
    tokens_a = normalize_text(answer_a)
    tokens_b = normalize_text(answer_b)
    ...
    overlap = sum(min(counts_a.get(token, 0), counts_b.get(token, 0)) for token in counts_a)
    if overlap == 0:
        return 0.0
...
def compute_lexical_redundancy(candidate: RetrievedDocument, docs: List[RetrievedDocument]) -> float:
    candidate_tokens = set(extract_content_tokens(candidate.text))
    if not candidate_tokens or not docs:
        return 0.0
    penalties: List[float] = []
    for doc in docs:
        doc_tokens = set(extract_content_tokens(doc.text))
        if not doc_tokens:
            continue
        union = candidate_tokens | doc_tokens
        penalties.append(len(candidate_tokens & doc_tokens) / len(union) if union else 0.0)
    return max(penalties) if penalties else 0.0


def build_diagnostic_perturbations(
    docs: List[RetrievedDocument],
    replacement_candidates: List[RetrievedDocument],
) -> List[List[RetrievedDocument]]:
    perturbations: List[List[RetrievedDocument]] = []
    if len(docs) > 1:
        perturbations.append(list(reversed(docs)))
    if docs and replacement_candidates:
        perturbations.append(docs[:-1] + [replacement_candidates[0]])
...
class StabilityAwareEvidenceSelector:
    def __init__(
        self,
        retriever: Retriever,
        generator: Generator,
    ...
    def _candidate_utility(
        ...
        utility = predicted_deficit_reduction - self.utility_rho * redundancy_penalty
        return CandidateUtility(
            candidate_doc_id=candidate.doc_id,
            utility=utility,
            delta_sufficiency=delta_sufficiency,
    ...
    def answer(self, query: Query, selection_strategy: str = "utility") -> dict:
        ...
        if not decision.sufficient:
            ...
            return {
                ...
                "features": decision.features,
                "sufficiency_score": decision.sufficiency_score,
                "anchoring_consistency": None,
                "post_selection_consistency": None,
                "stability_gain": None,
                "recovered": None,
                "diagnostic_generations": 0,
        ...
        if consistency >= self.stability_threshold or not candidates:
            return {
                "query": query.text,
                "decision": "answer_now",
                "reason": "answer_sufficient_stable" if candidates else "answer_sufficient_no_candidates",
                "used_docs": [doc.doc_id for doc in initial_docs],
                "initial_doc_ids": [doc.doc_id for doc in initial_docs],
                "final_doc_ids": [doc.doc_id for doc in initial_docs],
                "expanded_doc_ids": [],
                ...
                "post_selection_consistency": consistency,
                "stability_gain": 0.0,
                "recovered": consistency >= self.stability_threshold,
        ...
        if selection_strategy == "diagnose_then_expand":
            final_docs = pool[: self.expanded_k]
            post_perturbations = build_diagnostic_perturbations(final_docs, [])
            post_consistency, post_generations, answer = compute_anchoring_consistency(
                query,
                final_docs,
                self.generator,
                post_perturbations,
            ...
            return {
                ...
                "sufficiency_score": decision.sufficiency_score,
                "anchoring_consistency": consistency,
                "post_selection_consistency": post_consistency,
                "stability_gain": post_consistency - consistency,
                "recovered": post_consistency >= self.stability_threshold,
                "diagnostic_generations": diagnostic_generations + post_generations,
                "candidate_utility": None,
                "candidate_delta_sufficiency": None,
                "candidate_delta_consistency": None,
                "candidate_redundancy_penalty": None,
        ...
        return {
            ...
            "initial_doc_ids": [doc.doc_id for doc in initial_docs],
            "final_doc_ids": [doc.doc_id for doc in final_docs],
            "expanded_doc_ids": [selected.doc_id],
            ...
            "sufficiency_score": decision.sufficiency_score,
            "anchoring_consistency": consistency,
            "post_selection_consistency": selected_utility.post_consistency,
            "stability_gain": selected_utility.delta_consistency,
            "recovered": selected_utility.post_consistency >= self.stability_threshold,
            "diagnostic_generations": diagnostic_generations + candidate_scoring_generations,
            "candidate_utility": selected_utility.utility,
